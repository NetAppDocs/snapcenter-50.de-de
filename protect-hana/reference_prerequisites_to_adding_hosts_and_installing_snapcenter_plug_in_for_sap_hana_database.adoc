...
Um einen zusätzlichen Befehl für das SnapCenter-Plug-in für SAP HANA auszuführen, müssen Sie ihn in die Datei aufnehmen `allowed_commands.config` .
...
Benutzerdefinierter Pfad: `<Custome_Directory>\NetApp\SnapCenter\HANA\etc\allowed_commands.config` Windows-Host:
...
Um zusätzliche Befehle auf dem Plug-in-Host zuzulassen, öffnen Sie `allowed_commands.config` die Datei in einem Editor. Geben Sie jeden Befehl in einer eigenen Zeile ein. Die Groß-/Kleinschreibung wird nicht beachtet. Beispiel:
...
Wenn die `allowed_commands.config` Datei nicht vorhanden ist, werden die Befehle oder die Skriptausführung blockiert, und der Workflow schlägt mit dem folgenden Fehler fehl:
...
Wenn der Befehl oder das Skript nicht im vorhanden `allowed_commands.config`ist, wird die Ausführung des Befehls oder Skripts blockiert, und der Workflow schlägt mit dem folgenden Fehler fehl:
